-- LOCK CANONICAL tpc_invoices schema - FINAL fix for ALL missing columns
-- This ensures create_invoice RPC has ALL required columns

BEGIN;

-- Canonical purchase + totals fields
...
-- Canonical destination fields (THIS FIXES CURRENT ERROR)
...
-- Backfill treasury_address if null (server-side source of truth)
UPDATE public.tpc_invoices
...
-- Backfill tpc_amount from legacy candidates WITHOUT hardcoding (safe)
DO $$
DECLARE legacy_col text;
BEGIN
  SELECT c.column_name INTO legacy_col
  FROM information_schema.columns c
  WHERE c.table_schema='public' AND table_name='tpc_invoices'
  ...
  IF legacy_col IS NOT NULL THEN
    EXECUTE format(
      'UPDATE public.tpc_invoices SET tpc_amount = COALESCE(tpc_amount, %I::numeric) WHERE tpc_amount IS NULL;',
      legacy_col
    );
  END IF;
END $$;
