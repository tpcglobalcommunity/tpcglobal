...
export default function InviteAcceptPage({ lang }: { lang: Language }) {
  ...
  useEffect(() => {
    const acceptInvite = async () => {
      try {
        ...
        setError(err?.message || t('auth.invite.error') || 'Invitation failed');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <AuthShell
      ...
      subtitle={t('auth.invite.subtitle') || 'Join TPC Global'}
    >
      <div className="text-center">
        {loading && (
          <div className="space-y-4">
            ...
            <p className="text-white/70">
              {t('auth.invite.loading') || 'Processing invitation...'}
            </p>
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-xl font-semibold text-white mb-2">
                {t('auth.invite.success') || 'Welcome to TPC!'}
              </h3>
              <p className="text-white/70 mb-6">
                {t('auth.invite.successMessage') || 'Your invitation has been accepted. Welcome to TPC Global!'}
              </p>
            </div>
            <button
            ...
            >
              {t('auth.invite.continue') || 'Continue to Dashboard'}
            </button>
          </div>
        )}
        ...
        {error && (
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-xl font-semibold text-white mb-2">
                {t('auth.invite.failed') || 'Invitation Failed'}
              </h3>
              <p className="text-white/70 mb-6">
                {error}
              </p>
            ...
            <button
              ...
              className="w-full bg-[#F0B90B] text-black font-semibold py-3 px-6 rounded-xl hover:bg-[#F8D568] transition-colors"
            >
              {t('auth.invite.tryAgain') || 'Try Again'}
            </button>
          </div>
        )}
      </div>
